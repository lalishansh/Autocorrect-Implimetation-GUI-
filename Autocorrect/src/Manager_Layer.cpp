...
void Manager_Layer::OnAttach ()
{
	...
	m_MyImGuiStyle = ImGui::GetStyle ();
	Application::Get().GetWindow().SetOpacity (m_Settings_main._window_opacity);

	m_FontGlobalScale = ImGui::GetIO ().FontGlobalScale;
}
...
void ImGuiRenderLoadProgress (SymSpell_Dictionary *ptr, ImVec2 posn)
{
	if (ptr->progress.for_src != nullptr) {
		...
		HelpMarker ("(?)", "Load Progress of: %s %f %", ptr->progress.for_src, ptr->progress.percent ());
		ImGui::PopStyleVar (2);
		ImGui::End ();
	}
}
	...
	if (m_SettingsWindowOpen) {
		...
		if (ImGui::BeginTabBar ("Sections")) {
			if (ImGui::BeginTabItem ("Core")) {
				...
				m_SettingsChanged |= ImGui::Checkbox ("Unified Dictionary", &m_Settings_temp._unified_dictionary);
				ImGui::SameLine (); HelpMarker ("(?)", "When checked uses single instance of dictionary for every text entity");
				m_SettingsChanged |= ImGui::Checkbox ("Use Ctrl + Up/Dn instead of just Up/Dn for suggestions nav.", &m_Settings_temp._use_ctrl_for_suggestion_nav);
				ImGui::SameLine (); HelpMarker ("(?)", "When checked uses Ctrl + Up/Dn for trversing suggestions generated by dictionary.\nWhen Unchecked use only Up/Dn key for traversing suggestions");
				...
				if (ImGui::Button ("Add Extn") || (GLCore::Input::IsKeyPressed (HZ_KEY_ENTER) && new_extn[0]!='\0')) {
				...
				ImGui::SetNextItemWidth ((ImGui::GetContentRegionAvailWidth () - ImGui::GetCursorPosX () + ImGui::GetStyle ().ItemSpacing.x)*0.5f);
				ImGui::InputText ("<<", new_extn, 11, ImGuiInputTextFlags_CharsNoBlank | ImGuiInputTextFlags_NoUndoRedo );
				ImGui::SameLine (); HelpMarker ("(?)", "Add File Extensions to known Extension types. \n(Otherwise it'll always prompt whenever file is opened with unknown extension)");
				if (applied)
					idx = 0, new_extn[0] = '\0';
				...
				ImGui::SameLine (); HelpMarker ("(?)", "At Right Corner of Window a hover is shown \nwhenever special actions are performed on global scale.\nThis specifies duration for which it'll last before fading");
				...
				m_SettingsChanged |= ImGui::SliderInt ("Min Word Width", &m_Settings_temp._minWord_length_to_invoke_lookup, 2, 6);
				ImGui::SameLine (); HelpMarker ("(?)", "Min Word Width to invoke spell-suggestions");
			...
			if (ImGui::BeginTabItem ("Basic")) {
				...
				if (ImGui::SliderFloat ("FrameRounding", &style.FrameRounding, 0.0f, 12.0f, "%.0f"))
					style.GrabRounding = style.FrameRounding, m_SettingsChanged = true; // Make GrabRounding always the same value as FrameRounding
				{ bool border = (style.WindowBorderSize > 0.0f); if (ImGui::Checkbox ("WindowBorder", &border)) { style.WindowBorderSize = border ? 1.0f : 0.0f, m_SettingsChanged = true; } }
				ImGui::SameLine ();
				{ bool border = (style.FrameBorderSize > 0.0f);  if (ImGui::Checkbox ("FrameBorder", &border)) { style.FrameBorderSize = border ? 1.0f : 0.0f, m_SettingsChanged = true; } }
				ImGui::SameLine ();
				{ bool border = (style.PopupBorderSize > 0.0f);  if (ImGui::Checkbox ("PopupBorder", &border)) { style.PopupBorderSize = border ? 1.0f : 0.0f, m_SettingsChanged = true; } }
			...
			if (ImGui::BeginTabItem ("Advanced and fun")) {
					
				ImGui::Text ("Main");
				m_SettingsChanged |= ImGui::SliderFloat2 ("WindowPadding", (float *)&style.WindowPadding, 0.0f, 20.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat2 ("FramePadding", (float *)&style.FramePadding, 0.0f, 20.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat2 ("CellPadding", (float *)&style.CellPadding, 0.0f, 20.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat2 ("ItemSpacing", (float *)&style.ItemSpacing, 0.0f, 20.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("IndentSpacing", &style.IndentSpacing, 0.0f, 30.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("ScrollbarSize", &style.ScrollbarSize, 1.0f, 20.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("GrabMinSize", &style.GrabMinSize, 1.0f, 20.0f, "%.0f");
				ImGui::Text ("Borders");
				m_SettingsChanged |= ImGui::SliderFloat ("WindowBorderSize", &style.WindowBorderSize, 0.0f, 1.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("ChildBorderSize", &style.ChildBorderSize, 0.0f, 1.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("PopupBorderSize", &style.PopupBorderSize, 0.0f, 1.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("FrameBorderSize", &style.FrameBorderSize, 0.0f, 1.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("TabBorderSize", &style.TabBorderSize, 0.0f, 1.0f, "%.0f");
				ImGui::Text ("Rounding");
				m_SettingsChanged |= ImGui::SliderFloat ("WindowRounding", &style.WindowRounding, 0.0f, 12.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("ChildRounding", &style.ChildRounding, 0.0f, 12.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("FrameRounding", &style.FrameRounding, 0.0f, 12.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("PopupRounding", &style.PopupRounding, 0.0f, 12.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("ScrollbarRounding", &style.ScrollbarRounding, 0.0f, 12.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("GrabRounding", &style.GrabRounding, 0.0f, 12.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("LogSliderDeadzone", &style.LogSliderDeadzone, 0.0f, 12.0f, "%.0f");
				m_SettingsChanged |= ImGui::SliderFloat ("TabRounding", &style.TabRounding, 0.0f, 12.0f, "%.0f");
				ImGui::Text ("Alignment");
				m_SettingsChanged |= ImGui::SliderFloat2 ("WindowTitleAlign", (float *)&style.WindowTitleAlign, 0.0f, 1.0f, "%.2f");
				int window_menu_button_position = style.WindowMenuButtonPosition + 1;
				if (ImGui::Combo ("WindowMenuButtonPosition", (int *)&window_menu_button_position, "None\0Left\0Right\0"))
					style.WindowMenuButtonPosition = window_menu_button_position - 1, m_SettingsChanged = true;
				m_SettingsChanged |= ImGui::Combo ("ColorButtonPosition", (int *)&style.ColorButtonPosition, "Left\0Right\0");
				m_SettingsChanged |= ImGui::SliderFloat2 ("ButtonTextAlign", (float *)&style.ButtonTextAlign, 0.0f, 1.0f, "%.2f");
				ImGui::SameLine (); HelpMarker ("(?)", "Alignment applies when a button is larger than its text content.");
				m_SettingsChanged |= ImGui::SliderFloat2 ("SelectableTextAlign", (float *)&style.SelectableTextAlign, 0.0f, 1.0f, "%.2f");
			...
			if (ImGui::BeginTabItem ("Rendering")) {
				...
				if (ImGui::DragFloat ("window scale", &m_Settings_temp._window_scale, 0.005f, 0.4f, 2.0f, "%.2f", ImGuiSliderFlags_AlwaysClamp)) // Scale only this window
					ImGui::SetWindowFontScale (m_Settings_temp._window_scale), m_SettingsChanged = true;
				// TODO move to settings
				m_SettingsChanged |= ImGui::DragFloat ("Global Font Scale", &ImGui::GetIO().FontGlobalScale, 0.005f, 0.4f, 2.0f, "%.2f", ImGuiSliderFlags_AlwaysClamp); // Scale everything
				ImGui::PopItemWidth ();

				m_SettingsChanged |= ImGui::Checkbox ("Anti-aliased lines", &style.AntiAliasedLines);
				ImGui::SameLine ();
				HelpMarker ("(?)","When disabling anti-aliasing lines, you'll probably want to disable borders in your style as well.");

				m_SettingsChanged |= ImGui::Checkbox ("Anti-aliased lines use texture", &style.AntiAliasedLinesUseTex);
				ImGui::SameLine ();
				...
				m_SettingsChanged |= ImGui::Checkbox ("Anti-aliased fill", &style.AntiAliasedFill);
				ImGui::PushItemWidth (ImGui::GetFontSize () * 8);
				m_SettingsChanged |= ImGui::DragFloat ("Curve Tessellation Tolerance", &style.CurveTessellationTol, 0.02f, 0.10f, 10.0f, "%.2f");
				if (style.CurveTessellationTol < 0.10f) style.CurveTessellationTol = 0.10f;

				// When editing the "Circle Segment Max Error" value, draw a preview of its effect on auto-tessellated circles.
				m_SettingsChanged |= ImGui::DragFloat ("Circle Tessellation Max Error", &style.CircleTessellationMaxError, 0.005f, 0.10f, 5.0f, "%.2f", ImGuiSliderFlags_AlwaysClamp);
				...
				m_SettingsChanged |= ImGui::DragFloat ("Global Alpha", &style.Alpha, 0.005f, 0.20f, 1.0f, "%.2f"); // Not exposing zero here so user doesn't "lose" the UI (zero alpha clips all widgets). But application code could have a toggle to switch between zero and non-zero.
				ImGui::PopItemWidth ();
...
void Manager_Layer::TrySaveSettings()
{
	if (m_SettingsChanged) {
		...
		m_Settings_main._window_scale = m_Settings_temp._window_scale;
...
void Manager_Layer::DiscardSettings ()
{
	...
	ImGui::GetStyle () = m_MyImGuiStyle;
	
	m_Settings_temp._window_scale = m_Settings_main._window_scale;
	ImGui::SetWindowFontScale (m_Settings_main._window_scale);
	...
	ImGui::GetIO ().FontGlobalScale = m_FontGlobalScale;
}
